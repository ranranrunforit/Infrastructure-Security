...
|-- .editorconfig                   # UTF-8 and newline defaults
|-- .github/
...
1. Verifies commits are GitHub-verified.
2. Runs CodeQL when CodeQL-supported source files are present.
...
Both child apps set `CreateNamespace=true` so ArgoCD can create their target namespaces during sync.
